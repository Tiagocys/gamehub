...
async function fetchUserProfile({ supabaseUrl, supabaseAnonKey, userId }) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Supabase users query failed (${response.status}): ${payload}`);
  }
...
<body>
  <noscript>
    <p><a href="${escAppUrl}">Abrir perfil</a></p>
  </noscript>
</body>
</html>`;
}
...
export async function onRequestGet(context) {
  ...
  const appUrl = `${siteUrl}/user?id=${encodeURIComponent(userId)}`;
  const ogUrl = `${siteUrl}/og/user/${encodeURIComponent(userId)}`;
  ...
  try {
    const profile = await fetchUserProfile({ supabaseUrl, supabaseAnonKey, userId });

    if (!profile || String(profile.status || "active") !== "active") {
      const html = renderMetaPage({
        title: "Perfil não encontrado | Gimerr",
        description: "Este perfil foi removido, desativado ou não está disponível.",
    ...
    console.error("OG user route error:", err);
    const fallbackHtml = renderMetaPage({
      title: "Gimerr - Perfil público",
      description: "Confira perfis públicos da comunidade gamer no Gimerr.",
      imageUrl: normalizeImageUrl(DEFAULT_META_PATH, siteUrl),
      ogUrl,
      appUrl,
    });
    ...
    headers.set("x-og-debug", String(err?.message || "unknown_error").slice(0, 180));
